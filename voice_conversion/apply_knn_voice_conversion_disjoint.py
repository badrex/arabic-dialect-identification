...
os.environ['TORCH_HOME'] = project_dir + "pretrained_models"
...
output_dir = f"converted_{split}_ADI5_60_spkrs_cv_ara/"  + split + '/'
...
target_voices_dir = 'targets_60_arabic_cv/' 
...
# shuffle the target voices
random.shuffle(all_target_voices)
...
# in the list of 6 target voices, take 12 distinct voices for each dialect 
# to create a disjoint set of target voices for each dialect
dialect_to_target_voices = defaultdict()

idx = 0
# 12 voices for each dialect
for dialect in ['GLF', 'EGY', 'LAV', 'NOR', 'MSA']:
    dialect_to_target_voices[dialect] = all_target_voices[idx:idx+12]
    idx += 12
...
for dialect in ['GLF', 'EGY', 'LAV', 'NOR', 'MSA']:
    ...
    dialect_dir = f"ADI5/{split}/" + dialect + '/'
    ...
    for audio_clip in tqdm(audio_clip_path):
        ...
        target_voice = random.choice(dialect_to_target_voices[dialect])
